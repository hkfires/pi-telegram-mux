...
在绑定的 Telegram 话题中发送照片或图片文件，并在附言中写下任务说明。也可以不写附言，插件不会添加默认提示词。

需要同时处理多张图片时，请使用 Telegram 相册一次发送（最多 10 张）。相册作为一项任务提交，图片按 `[Image#1]`、`[Image#2]` 等编号。分开发送的图片不会自动合并。

**注意事项：**

- 使用 `/model` 选择支持图片的模型。支持 JPEG、PNG、GIF、WebP，其他附件暂不支持。
- 插件不限制单图或相册文件大小，但 Telegram 和模型服务可能拒绝接收。会话繁忙时，新输入可能被暂时拒绝。
- 相册中有图片下载或读取失败时，整组不执行；晚到的图片会提示，不会另起任务。
- 交给 Pi 的图片会在本地保留，不会自动删除。默认目录为 `~/.pi/agent/pi-telegram-mux/media/`；设置了 `PI_CODING_AGENT_DIR` 时，位于该目录下的 `pi-telegram-mux/media/`。请手动清理不再需要的图片以释放磁盘空间；删除后无法再通过旧路径读取。

**升级后请重启所有 Pi 实例，避免新旧版本混用。**

### 手机端命令

在话题中直接发送文字即可提交任务。以下命令会自动出现在授权用户的 Telegram `/` 菜单中，选择即可使用：
